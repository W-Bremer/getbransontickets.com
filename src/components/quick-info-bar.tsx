...
export function QuickInfoBar({
  ...
  return (
    <div className="rounded-xl bg-[#F6F4EF] px-4 py-3 sm:px-6 sm:py-4">
      <div className="flex flex-wrap items-center gap-y-3">
        {items.map((item, i) => (
          <div key={item.label} className="flex items-center">
            {i > 0 && (
              <div className="mx-3 hidden h-6 w-px bg-[#1A1614]/15 sm:mx-4 sm:block" />
            )}
            <div className="flex items-center gap-2">
              <item.icon className="h-4.5 w-4.5 shrink-0 text-[#13264D]" />
            ...
            {i < items.length - 1 && (
              <div className="mx-3 block h-6 w-px bg-[#1A1614]/15 sm:mx-4 sm:hidden" />
            )}
          </div>
        ))}
      </div>
